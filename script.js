...
const floatingCheckButton = document.getElementById('floatingCheckButton');
const mainCheckButton = document.getElementById('mainCheckButton');
...
function setupEventListeners() {
    ...
    window.addEventListener('scroll', toggleFloatingCheckButton);
    
    // Add keyboard navigation for quick years
    document.querySelectorAll('.quick-year').forEach(year => {
...
function checkBSStage() {
    ...
    `;
    topResult.className = `top-result-section active ${resultClass}`;

    // Scroll to top result
    topResult.scrollIntoView({ behavior: 'smooth' });
}

// Toggle floating check button visibility
function toggleFloatingCheckButton() {
    const mainButtonRect = mainCheckButton.getBoundingClientRect();
    
    // Show floating button if main button is not in viewport
    if (mainButtonRect.top > window.innerHeight || mainButtonRect.bottom < 0) {
        floatingCheckButton.style.display = 'flex';
    } else {
        floatingCheckButton.style.display = 'none';
    }
}
